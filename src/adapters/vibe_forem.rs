...
const VIBE_FOREM_API_BASE: &str = "https://vibe.forem.com/api";
const PER_PAGE: u32 = 100;

/// Article data from /articles/me/all endpoint (includes full content)
#[derive(Debug, Deserialize, Clone)]
struct VibeForemArticleListItem {
    id: u64,
    title: String,
...
#[derive(Debug, Deserialize)]
struct VibeForemSeries {
    name: String,
}

pub struct VibeForemPuller {
    client: reqwest::Client,
    api_key: String,
    /// Cache of articles fetched from list endpoint (for drafts that can't be fetched individually)
    article_cache: RwLock<HashMap<String, VibeForemArticleListItem>>,
}

impl VibeForemPuller {
    ...
    async fn fetch_page(&self, page: u32) -> Result<Vec<VibeForemArticleListItem>> {
        let url = format!("{VIBE_FOREM_API_BASE}/articles/me/all?page={page}&per_page={PER_PAGE}");
        ...
        if !response.status().is_success() {
            ...
            return Err(PullError::Api(format!(
                "Vibe Forem API returned {status}: {body}"
            )));
        }
...
#[async_trait]
impl Puller for VibeForemPuller {
    ...
    async fn fetch_article(&self, id: &str) -> Result<PulledArticle> {
        ...
        let url = format!("{VIBE_FOREM_API_BASE}/articles/{id}");
        ...
        let article: VibeForemArticle = response.json().await?;
